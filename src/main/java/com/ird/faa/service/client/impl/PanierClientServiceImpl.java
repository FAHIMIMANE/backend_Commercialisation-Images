...
import com.ird.faa.service.client.facade.PanierClientService;
import com.ird.faa.service.client.facade.PanierItemClientService;
...
@Service
public class PanierClientServiceImpl extends AbstractServiceImpl<Panier> implements PanierClientService {

    @Autowired
    private PanierDao panierDao;

    @Autowired
    private PanierItemClientService panierItemService;
    @Autowired
    private EtatPanierClientService etatPanierService;
    ...
    private void calcTotalPanier(Panier panier) {
        BigDecimal total = BigDecimal.ZERO;
        List<PanierItem> panierItems = panier.getPanierItems();
        if (panierItems != null) {
            for (PanierItem panierItem : panierItems) {
                total = total.add(panierItem.getPrix());
            }
        }
